...
import com.minglang.suiuu.animation.MenuAnimationHandler;
...
public class SlideInAnimationHandler extends MenuAnimationHandler {
	...
	public void animateMenuOpening(Point center) {
		...
		for (int i = 0; i < menu.getSubActionItems().size(); i++) {
			...
			FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) menu
					.getSubActionItems().get(i).view.getLayoutParams();
			params.setMargins(menu.getSubActionItems().get(i).x, menu
					.getSubActionItems().get(i).y + DIST_Y, 0, 0);
			menu.getSubActionItems().get(i).view.setLayoutParams(params);

			// PropertyValuesHolder pvhX =
			// PropertyValuesHolder.ofFloat(View.TRANSLATION_X,
			// menu.getSubActionItems().get(i).x/* - center.x +
			// menu.getSubActionItems().get(i).width / 2*/);
			PropertyValuesHolder pvhY = PropertyValuesHolder.ofFloat(
					View.TRANSLATION_Y, -DIST_Y);
			...
			PropertyValuesHolder pvhA = PropertyValuesHolder.ofFloat(
					View.ALPHA, 1);

			final ObjectAnimator animation = ObjectAnimator
					.ofPropertyValuesHolder(
							menu.getSubActionItems().get(i).view, pvhY, pvhA);
			animation.setDuration(DURATION);
			animation.setInterpolator(new DecelerateInterpolator());
			animation.addListener(new SubActionItemAnimationListener(menu
					.getSubActionItems().get(i), ActionType.OPENING));

			if (i == 0) {
				lastAnimation = animation;
			}

			animation.setStartDelay(Math.abs(menu.getSubActionItems().size()
					/ 2 - i)
					* LAG_BETWEEN_ITEMS);
			animation.start();
		}
	...
	public void animateMenuClosing(Point center) {
		...
		for (int i = 0; i < menu.getSubActionItems().size(); i++) {
			...
			animation.addListener(new SubActionItemAnimationListener(menu
					.getSubActionItems().get(i), ActionType.CLOSING));
			...
			if (i <= menu.getSubActionItems().size() / 2) {
				...
				animation.setStartDelay((menu.getSubActionItems().size() - i)
						* LAG_BETWEEN_ITEMS);
			}
			animation.start();
		}
	...
	protected class SubActionItemAnimationListener implements
			Animator.AnimatorListener {

		private FloatingActionMenu.Item subActionItem;
		private ActionType actionType;

		public SubActionItemAnimationListener(
				FloatingActionMenu.Item subActionItem, ActionType actionType) {
